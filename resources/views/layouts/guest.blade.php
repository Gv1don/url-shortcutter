...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'ShortCutter') }}</title>
        ...
        <div class="min-h-screen flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-gradient-to-br from-indigo-50 via-white to-purple-50">
            <div class="mb-6">
                <a href="/" class="flex items-center gap-3">
                    ...
                    <span class="text-2xl font-bold text-gray-900">{{ config('app.name', 'ShortCutter') }}</span>
                </a>
            </div>

            <div class="w-full sm:max-w-md px-8 py-8 bg-white shadow-xl shadow-gray-200/50 sm:rounded-2xl">
                {{ $slot }}
            </div>
        </div>
    </body>
